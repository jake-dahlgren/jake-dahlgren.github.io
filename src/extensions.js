import { html, PolymerElement, templatize, Polymer, Base, dom, dedupingMixin, useShadow, dashToCamelCase, mixinBehaviors, Templatizer, OptionalMutableDataBehavior, animationFrame, microTask, idlePeriod, flush, Debouncer, enqueueDebouncer, matches, translate, FlattenedNodesObserver, afterNextRender, timeOut } from "chrome://resources/polymer/v3_0/polymer/polymer_bundled.min.js"; import "chrome://extensions/strings.m.js"; import { loadTimeData } from "chrome://resources/js/load_time_data.m.js"; import { addWebUiListener, removeWebUiListener } from "chrome://resources/js/cr.js";
/**
@license
...
function assert$1(value, message) { if (value) { return } throw new Error("Assertion failed" + (message ? `: ${message}` : "")) } function assertInstanceof(value, type, message) { if (value instanceof type) { return } throw new Error(message || `Value ${value} is not of type ${type.name || typeof type}`) } function assertNotReached$1(message = "Unreachable code hit") { assert$1(false, message) }
...
class CrDrawerElement extends PolymerElement { static get is() { return "cr-drawer" } static get template() { return getTemplate$Q() } static get properties() { return { heading: String, show_: { type: Boolean, reflectToAttribute: true }, align: { type: String, value: "ltr", reflectToAttribute: true } } } fire_(eventName, detail) { this.dispatchEvent(new CustomEvent(eventName, { bubbles: true, composed: true, detail: detail })) } get open() { return this.$.dialog.open } set open(_value) { assertNotReached$1("Cannot set |open|.") } toggle() { if (this.open) { this.cancel() } else { this.openDrawer() } } openDrawer() { if (this.open) { return } this.$.dialog.showModal(); this.show_ = true; this.fire_("cr-drawer-opening"); listenOnce(this.$.dialog, "transitionend", (() => { this.fire_("cr-drawer-opened") })) } dismiss_(cancel) { if (!this.open) { return } this.show_ = false; listenOnce(this.$.dialog, "transitionend", (() => { this.$.dialog.close(cancel ? "canceled" : "closed") })) } cancel() { this.dismiss_(true) } close() { this.dismiss_(false) } wasCanceled() { return !this.open && this.$.dialog.returnValue === "canceled" } onContainerTap_(event) { event.stopPropagation() } onDialogTap_() { this.cancel() } onDialogCancel_(event) { event.preventDefault(); this.cancel() } onDialogClose_() { this.fire_("close") } } customElements.define(CrDrawerElement.is, CrDrawerElement);
// Copyright 2016 The Chromium Authors
...
`.content); styleMod$8.register("cr-hidden-style"); function getTemplate$P() {
...
let toastManagerInstance = null; function getToastManager() { assert$1(toastManagerInstance); return toastManagerInstance } function setInstance(instance) { assert$1(!instance || !toastManagerInstance); toastManagerInstance = instance } class CrToastManagerElement extends PolymerElement { static get is() { return "cr-toast-manager" } static get template() { return getTemplate$O() } static get properties() { return { duration: { type: Number, value: 0 } } } get isToastOpen() { return this.$.toast.open } get slottedHidden() { return this.$.slotted.hidden } connectedCallback() { super.connectedCallback(); setInstance(this) } disconnectedCallback() { super.disconnectedCallback(); setInstance(null) } show(label, hideSlotted = false) { this.$.content.textContent = label; this.showInternal_(hideSlotted) } showForStringPieces(pieces, hideSlotted = false) { const content = this.$.content; content.textContent = ""; pieces.forEach((function (p) { if (p.value.length === 0) { return } const span = document.createElement("span"); span.textContent = p.value; if (p.collapsible) { span.classList.add("collapsible") } content.appendChild(span) })); this.showInternal_(hideSlotted) } showInternal_(hideSlotted) { this.$.slotted.hidden = hideSlotted; this.$.toast.show() } hide() { this.$.toast.hide() } } customElements.define(CrToastManagerElement.is, CrToastManagerElement);
/**
@license
Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at
...
<!--_html_template_end_-->`}
// Copyright 2016 The Chromium Authors
class CrToolbarElement extends PolymerElement { static get is() { return "cr-toolbar" } static get template() { return getTemplate$L() } static get properties() { return { pageName: String, searchPrompt: String, clearLabel: String, menuLabel: String, spinnerActive: Boolean, showMenu: { type: Boolean, value: false }, showSearch: { type: Boolean, value: true }, autofocus: { type: Boolean, value: false, reflectToAttribute: true }, narrow: { type: Boolean, reflectToAttribute: true, readonly: true, notify: true }, narrowThreshold: { type: Number, value: 900 }, alwaysShowLogo: { type: Boolean, value: false, reflectToAttribute: true }, showingSearch_: { type: Boolean, reflectToAttribute: true } } } getSearchField() { return this.$.search } onMenuTap_() { this.dispatchEvent(new CustomEvent("cr-toolbar-menu-tap", { bubbles: true, composed: true })) } focusMenuButton() { requestAnimationFrame((() => { const menuButton = this.shadowRoot.querySelector("#menuButton"); if (menuButton) { menuButton.focus() } })) } isMenuFocused() { return !!this.shadowRoot.activeElement && this.shadowRoot.activeElement.id === "menuButton" } } customElements.define(CrToolbarElement.is, CrToolbarElement); function getTemplate$K() {
...
<!--_html_template_end_-->`}
// Copyright 2016 The Chromium Authors
const CrSearchFieldElementBase = CrSearchFieldMixin(PolymerElement); class CrSearchFieldElement extends CrSearchFieldElementBase { static get is() { return "cr-search-field" } static get template() { return getTemplate$G() } static get properties() { return { autofocus: { type: Boolean, value: false } } } getSearchInput() { return this.$.searchInput } onTapClear_() { this.setValue(""); setTimeout((() => { this.$.searchInput.focus() })) } } customElements.define(CrSearchFieldElement.is, CrSearchFieldElement);
/**
@license
...
part of the polymer project is also subject to an additional IP rights grant
found at http://polymer.github.io/PATENTS.txt
*/var IOS = navigator.userAgent.match(/iP(?:hone|ad;(?: U;)? CPU) OS (\d+)/); var IOS_TOUCH_SCROLLING = IOS && IOS[1] >= 8; var DEFAULT_PHYSICAL_COUNT = 3; var HIDDEN_Y = "-10000px"; var SECRET_TABINDEX = -100; Polymer({
    _template: html`
    <style>
      :host {
        display: block;
...
<!--_html_template_end_-->`}
// Copyright 2019 The Chromium Authors
const ARG_URL_PLACEHOLDER = "<arg_url>"; const ARG_URL_PLACEHOLDER_REGEX = /"<arg_url>"/g; class ActivityLogStreamItemElement extends PolymerElement { static get is() { return "activity-log-stream-item" } static get template() { return getTemplate$E() } static get properties() { return { data: Object, argsList_: { type: Array, computed: "computeArgsList_(data.args)" }, isExpandable_: { type: Boolean, computed: "computeIsExpandable_(data)" } } } computeIsExpandable_() { return this.hasPageUrl_() || this.hasArgs_() || this.hasWebRequestInfo_() } getFormattedTime_() { const activityDate = new Date(this.data.timestamp); const timeString = activityDate.toLocaleTimeString(undefined, { hour12: false, hour: "2-digit", minute: "2-digit", second: "2-digit" }); const ms = activityDate.getMilliseconds().toString().padStart(3, "0"); return `${timeString}.${ms}` } hasPageUrl_() { return !!this.data.pageUrl } hasArgs_() { return this.argsList_.length > 0 } hasWebRequestInfo_() { return !!this.data.webRequestInfo && this.data.webRequestInfo !== "{}" } computeArgsList_() { const parsedArgs = JSON.parse(this.data.args); if (!Array.isArray(parsedArgs)) { return [] } return parsedArgs.map(((arg, i) => ({ arg: JSON.stringify(arg).replace(ARG_URL_PLACEHOLDER_REGEX, `"${this.data.argUrl}"`), index: i + 1 }))) } onExpandClick_() { if (this.isExpandable_) { this.set("data.expanded", !this.data.expanded); this.dispatchEvent(new CustomEvent("resize-stream")) } } } customElements.define(ActivityLogStreamItemElement.is, ActivityLogStreamItemElement); function getTemplate$D() {
...
const ExtensionsActivityLogElementBase = I18nMixin(CrContainerShadowMixin(PolymerElement)); class ExtensionsActivityLogElement extends ExtensionsActivityLogElementBase { static get is() { return "extensions-activity-log" } static get template() { return getTemplate$z() } static get properties() { return { extensionInfo: Object, delegate: Object, selectedSubpage_: { type: Number, value: -1, observer: "onSelectedSubpageChanged_" }, tabNames_: { type: Array, value: () => [loadTimeData.getString("activityLogHistoryTabHeading"), loadTimeData.getString("activityLogStreamTabHeading")] } } } ready() { super.ready(); this.addEventListener("view-enter-start", this.onViewEnterStart_); this.addEventListener("view-exit-finish", this.onViewExitFinish_) } onViewEnterStart_() { this.selectedSubpage_ = 0; afterNextRender(this, (() => focusWithoutInk(this.$.closeButton))) } onViewExitFinish_() { this.selectedSubpage_ = -1; const activityLogStream = this.shadowRoot.querySelector("activity-log-stream"); if (activityLogStream) { activityLogStream.clearStream() } } getActivityLogHeading_() { const headingName = this.extensionInfo.isPlaceholder ? this.i18n("missingOrUninstalledExtension") : this.extensionInfo.name; return this.i18n("activityLogPageHeading", headingName) } isHistoryTabSelected_() { return this.selectedSubpage_ === 0 } isStreamTabSelected_() { return this.selectedSubpage_ === 1 } onSelectedSubpageChanged_(newTab, oldTab) { const activityLogStream = this.shadowRoot.querySelector("activity-log-stream"); if (activityLogStream) { if (newTab === 1) { activityLogStream.startStream() } else if (oldTab === 1) { activityLogStream.pauseStream() } } } onCloseButtonTap_() { if (this.extensionInfo.isPlaceholder) { navigation.navigateTo({ page: Page.LIST }) } else { navigation.navigateTo({ page: Page.DETAILS, extensionId: this.extensionInfo.id }) } } } customElements.define(ExtensionsActivityLogElement.is, ExtensionsActivityLogElement); function getTemplate$y() {
    return html`<!--_html_template_start_--><style include="cr-actionable-row-style cr-shared-style cr-hidden-style">:host{box-sizing:border-box;flex:1;font-family:inherit;font-size:100%;line-height:154%;min-height:var(--cr-section-min-height);padding:0}:host(:not([embedded])){padding:0 var(--cr-section-padding)}#startIcon{--iron-icon-fill-color:var(--cr-link-row-start-icon-color,
        var(--google-grey-700));display:flex;flex-shrink:0;padding-inline-end:var(--cr-icon-button-margin-start);width:var(--cr-link-row-icon-width,var(--cr-icon-size))}@media (prefers-color-scheme:dark){#startIcon{--iron-icon-fill-color:var(--cr-link-row-start-icon-color,
          var(--google-grey-500))}}#labelWrapper{flex:1;flex-basis:.000000001px;padding-bottom:var(--cr-section-vertical-padding);padding-top:var(--cr-section-vertical-padding);text-align:start}#label,#subLabel{display:flex}</style>
...
<slot></slot>
<cr-icon-button id="icon" iron-icon="[[getIcon_(external)]]" role="link" aria-roledescription$="[[roleDescription]]" part="icon" aria-labelledby="label subLabel" disabled="[[disabled]]">
</cr-icon-button>
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
class CrLinkRowElement extends PolymerElement { static get is() { return "cr-link-row" } static get template() { return getTemplate$y() } static get properties() { return { startIcon: { type: String, value: "" }, label: { type: String, value: "" }, subLabel: { type: String, value: "" }, disabled: { type: Boolean, reflectToAttribute: true }, external: { type: Boolean, value: false }, usingSlottedLabel: { type: Boolean, value: false }, roleDescription: String, hideLabelWrapper_: { type: Boolean, computed: "computeHideLabelWrapper_(label, usingSlottedLabel)" } } } focus() { this.$.icon.focus() } computeHideLabelWrapper_() { return !(this.label || this.usingSlottedLabel) } getIcon_() { return this.external ? "cr:open-in-new" : "cr:arrow-right" } } customElements.define(CrLinkRowElement.is, CrLinkRowElement); function getTemplate$x() {
    return html`<!--_html_template_start_-->    <style>:host{--cr-toggle-checked-bar-color:var(--google-blue-600);--cr-toggle-checked-button-color:var(--google-blue-600);--cr-toggle-checked-ripple-color:rgba(var(--google-blue-600-rgb), .2);--cr-toggle-unchecked-bar-color:var(--google-grey-400);--cr-toggle-unchecked-button-color:white;--cr-toggle-unchecked-ripple-color:rgba(var(--google-grey-600-rgb), .15);-webkit-tap-highlight-color:transparent;cursor:pointer;display:block;min-width:34px;outline:0;position:relative;width:34px}@media (forced-colors:active){:host{forced-color-adjust:none}}@media (prefers-color-scheme:dark){:host{--cr-toggle-checked-bar-color:var(--google-blue-300);--cr-toggle-checked-button-color:var(--google-blue-300);--cr-toggle-checked-ripple-color:rgba(var(--google-blue-300-rgb), .4);--cr-toggle-unchecked-bar-color:var(--google-grey-500);--cr-toggle-unchecked-button-color:var(--google-grey-300);--cr-toggle-unchecked-ripple-color:rgba(var(--google-grey-300-rgb), .4)}}:host([dark]){--cr-toggle-checked-bar-color:var(--google-blue-300);--cr-toggle-checked-button-color:var(--google-blue-300);--cr-toggle-checked-ripple-color:rgba(var(--google-blue-300-rgb), .4);--cr-toggle-unchecked-bar-color:var(--google-grey-500);--cr-toggle-unchecked-button-color:var(--google-grey-300);--cr-toggle-unchecked-ripple-color:rgba(var(--google-grey-300-rgb), .4)}:host([disabled]){cursor:initial;opacity:var(--cr-disabled-opacity);pointer-events:none}#bar{background-color:var(--cr-toggle-unchecked-bar-color);border-radius:8px;height:12px;left:3px;position:absolute;top:2px;transition:background-color linear 80ms;width:28px;z-index:0}:host([checked]) #bar{background-color:var(--cr-toggle-checked-bar-color);opacity:var(--cr-toggle-checked-bar-opacity,.5)}#knob{background-color:var(--cr-toggle-unchecked-button-color);border-radius:50%;box-shadow:var(--cr-toggle-box-shadow,0 1px 3px 0 rgba(0,0,0,.4));display:block;height:16px;position:relative;transition:transform linear 80ms,background-color linear 80ms;width:16px;z-index:1}:host([checked]) #knob{background-color:var(--cr-toggle-checked-button-color);transform:translate3d(18px,0,0)}:host-context([dir=rtl]):host([checked]) #knob{transform:translate3d(-18px,0,0)}paper-ripple{--paper-ripple-opacity:1;color:var(--cr-toggle-unchecked-ripple-color);height:var(--cr-toggle-ripple-diameter,40px);left:50%;outline:var(--cr-toggle-ripple-ring,none);pointer-events:none;position:absolute;top:50%;transform:translate(-50%,-50%);transition:color linear 80ms;width:var(--cr-toggle-ripple-diameter,40px)}:host([checked]) paper-ripple{color:var(--cr-toggle-checked-ripple-color)}:host-context([dir=rtl]) paper-ripple{left:auto;right:50%;transform:translate(50%,-50%)}</style>
    <span id="bar"></span>
    <span id="knob"></span>
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
...
`; template$1.setAttribute("style", "display: none;"); document.head.appendChild(template$1.content); function getTemplate$v() {
...
<!--_html_template_end_-->`}
// Copyright 2022 The Chromium Authors
const ExtensionsRestrictedSitesDialogElementBase = I18nMixin(PolymerElement); class ExtensionsRestrictedSitesDialogElement extends ExtensionsRestrictedSitesDialogElementBase { static get is() { return "extensions-restricted-sites-dialog" } static get template() { return getTemplate$u() } static get properties() { return { firstRestrictedSite: { type: String, value: "" } } } isOpen() { return this.$.dialog.open } wasConfirmed() { return this.$.dialog.getNative().returnValue === "success" } onCancelClick_() { this.$.dialog.cancel() } onSubmitClick_() { this.$.dialog.close() } getDialogTitle_() { return this.i18n("matchingRestrictedSitesTitle", this.firstRestrictedSite) } getDialogWarning_() { return this.i18n("matchingRestrictedSitesWarning", this.firstRestrictedSite) } } customElements.define(ExtensionsRestrictedSitesDialogElement.is, ExtensionsRestrictedSitesDialogElement); function getTemplate$t() {
...
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
class ExtensionsToggleRowElement extends PolymerElement { static get is() { return "extensions-toggle-row" } static get template() { return getTemplate$t() } static get properties() { return { checked: Boolean, disabled: Boolean } } fire_(eventName, detail) { this.dispatchEvent(new CustomEvent(eventName, { bubbles: true, composed: true, detail: detail })) } getLabel() { return this.$.label } onNativeClick_(e) { e.stopPropagation() } onNativeChange_(e) { e.stopPropagation(); this.$.crToggle.checked = this.$.native.checked; this.checked = this.$.native.checked; this.fire_("change", this.checked) } onCrToggleChange_(e) { e.stopPropagation(); this.$.native.checked = e.detail; this.fire_("change", this.checked) } } customElements.define(ExtensionsToggleRowElement.is, ExtensionsToggleRowElement); function getTemplate$s() {
...
<div id="section-heading" hidden$="[[enableEnhancedSiteControls]]">
  ...
  <a id="linkIconButton" aria-label="Learn more" href="https://support.google.com/chrome?p=enable_extensions" target="_blank" on-click="onLearnMoreClick_">
...
<div class="toggle-section">
  <extensions-toggle-row checked="[[allowedOnAllHosts_(permissions.*)]]" id="allHostsToggle" on-change="onAllHostsToggleChanged_">
    ...
    <a id="linkIconButton" aria-label="Learn more" href="https://support.google.com/chrome?p=enable_extensions" target="_blank" on-click="onLearnMoreClick_" hidden$="[[!enableEnhancedSiteControls]]">
      <iron-icon icon="cr:help-outline"></iron-icon>
    </a>
...
<!--_html_template_end_-->`} function getTemplate$q() {
...
<!--_html_template_end_-->`}
// Copyright 2022 The Chromium Authors
const sitePermissionsPatternRegExp = new RegExp("^" + "((http|https)://)?" + "([a-z0-9\\.-]+\\.[a-z0-9]+|localhost)" + "(:[0-9]+)?" + "$"); function getSitePermissionsPatternFromSite(site) { const res = sitePermissionsPatternRegExp.exec(site); assert$1(res); const scheme = res[1] || "https://"; const host = res[3]; const port = res[4] || ""; return scheme + host + port } class SitePermissionsEditUrlDialogElement extends PolymerElement { static get is() { return "site-permissions-edit-url-dialog" } static get template() { return getTemplate$q() } static get properties() { return { delegate: Object, siteSet: String, siteToEdit: { type: String, value: null }, site_: { type: String, value: "" }, inputValid_: { type: Boolean, value: true } } } connectedCallback() { super.connectedCallback(); if (this.siteToEdit !== null) { this.site_ = this.siteToEdit; this.validate_() } } validate_() { this.inputValid_ = this.site_.trim().length === 0 || sitePermissionsPatternRegExp.test(this.site_) } computeDialogTitle_() { return loadTimeData.getString(this.siteToEdit === null ? "sitePermissionsAddSiteDialogTitle" : "sitePermissionsEditSiteDialogTitle") } computeSubmitButtonDisabled_() { return !this.inputValid_ || this.site_.trim().length === 0 } computeSubmitButtonLabel_() { return loadTimeData.getString(this.siteToEdit === null ? "add" : "save") } onCancel_() { this.$.dialog.cancel() } onSubmit_() { const pattern = getSitePermissionsPatternFromSite(this.site_); if (this.siteToEdit !== null) { this.handleEdit_(pattern) } else { this.handleAdd_(pattern) } } handleEdit_(pattern) { assert$1(this.siteToEdit); if (pattern === this.siteToEdit) { this.$.dialog.close(); return } this.delegate.removeUserSpecifiedSites(this.siteSet, [this.siteToEdit]).then((() => { this.addUserSpecifiedSite_(pattern) })) } handleAdd_(pattern) { assert$1(!this.siteToEdit); this.addUserSpecifiedSite_(pattern) } addUserSpecifiedSite_(pattern) { this.delegate.addUserSpecifiedSites(this.siteSet, [pattern]).then((() => { this.$.dialog.close() }), (() => { this.inputValid_ = false })) } } customElements.define(SitePermissionsEditUrlDialogElement.is, SitePermissionsEditUrlDialogElement);
// Copyright 2022 The Chromium Authors
...
`.content); styleMod.register("md-select"); function getTemplate$n() {
  ...
  <div id="permissions-mode">
    <div id="section-heading">
      <div id="section-heading-heading">
        ...
        <a class="link-icon-button" aria-label="Learn more" href="https://support.google.com/chrome?p=enable_extensions" target="_blank" on-click="onLearnMoreClick_">
          <iron-icon icon="cr:help-outline"></iron-icon>
        </a>
      </div>
      <div>
...
<div class="page-container" id="container">
  <div class="page-content">
    ...
    <cr-link-row class="hr" id="extensionsOptions" disabled="[[!isEnabled_(data.state)]]" hidden="[[!shouldShowOptionsLink_(data.*)]]" label="Extension options" on-click="onExtensionOptionsTap_" external></cr-link-row>
    ...
    <cr-link-row class="hr" hidden="[[!data.manifestHomePageUrl.length]]" id="extensionWebsite" label="Open extension website" on-click="onExtensionWebSiteTap_" external></cr-link-row>
    <cr-link-row class="hr" hidden="[[!data.webStoreUrl.length]]" id="viewInStore" label="View in Chrome Web Store" on-click="onViewInStoreTap_" external></cr-link-row>
    <div class="section hr">
      <div class="section-title" role="heading" aria-level="2">
        Source
      </div>
...
<!--_html_template_end_-->`}
// Copyright 2016 The Chromium Authors
class ExtensionsDropOverlayElement extends PolymerElement { constructor() { super(); this.hidden = true; const dragTarget = document.documentElement; this.dragWrapperHandler_ = new DragAndDropHandler(true, dragTarget); dragTarget.addEventListener("extension-drag-started", (() => { this.hidden = false })); dragTarget.addEventListener("extension-drag-ended", (() => { this.hidden = true })); dragTarget.addEventListener("drag-and-drop-load-error", (e => { this.dispatchEvent(new CustomEvent("load-error", { bubbles: true, composed: true, detail: e.detail })) })); this.dragWrapper_ = new DragWrapper(dragTarget, this.dragWrapperHandler_) } static get is() { return "extensions-drop-overlay" } static get template() { return getTemplate$l() } static get properties() { return { dragEnabled: { type: Boolean, observer: "dragEnabledChanged_" } } } dragEnabledChanged_(dragEnabled) { this.dragWrapperHandler_.dragEnabled = dragEnabled } } customElements.define(ExtensionsDropOverlayElement.is, ExtensionsDropOverlayElement);
/**
@license
Copyright (c) 2015 The Polymer Project Authors. All rights reserved.
This code may only be used under the BSD style license found at
...
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
class ExtensionsInstallWarningsDialogElement extends PolymerElement { static get is() { return "extensions-install-warnings-dialog" } static get template() { return getTemplate$i() } static get properties() { return { installWarnings: Array } } connectedCallback() { super.connectedCallback(); this.$.dialog.showModal() } onOkTap_() { this.$.dialog.close() } } customElements.define(ExtensionsInstallWarningsDialogElement.is, ExtensionsInstallWarningsDialogElement);
// Copyright 2021 The Chromium Authors
const WebUiListenerMixin = dedupingMixin((superClass => { class WebUiListenerMixin extends superClass { constructor() { super(...arguments); this.webUiListeners_ = [] } addWebUIListener(eventName, callback) { this.webUiListeners_.push(addWebUiListener(eventName, callback)) } disconnectedCallback() { super.disconnectedCallback(); while (this.webUiListeners_.length > 0) { removeWebUiListener(this.webUiListeners_.pop()) } } } return WebUiListenerMixin })); function getTemplate$h() {
...
`; document.head.appendChild(template.content); function getTemplate$g() {
      ...
      <div class="card-controls">
        <template is="dom-repeat" items="[[item.commands]]" as="command">
          <div class="command-entry" command="[[command]]">
            ...
            <extensions-shortcut-input delegate="[[delegate]]" item="[[item.id]]" shortcut="[[command.keybinding]]" command-name="[[command.name]]">
            </extensions-shortcut-input>
            
            <select class="md-select" on-change="onScopeChanged_" disabled$="[[computeScopeDisabled_(command)]]" value="[[
...
<!--_html_template_end_-->`}
// Copyright 2016 The Chromium Authors
const ExtensionsKeyboardShortcutsElementBase = CrContainerShadowMixin(PolymerElement); class ExtensionsKeyboardShortcutsElement extends ExtensionsKeyboardShortcutsElementBase { static get is() { return "extensions-keyboard-shortcuts" } static get template() { return getTemplate$d() } static get properties() { return { delegate: Object, items: Array, CommandScope_: { type: Object, value: chrome.developerPrivate.CommandScope } } } ready() { super.ready(); this.addEventListener("view-enter-start", this.onViewEnter_) } onViewEnter_() { chrome.metricsPrivate.recordUserAction("Options_ExtensionCommands") } calculateShownItems_() { return this.items.filter((function (item) { return item.commands.length > 0 })) } hasKeybinding_(keybinding) { return !!keybinding } computeScopeDisabled_(command) { return command.isExtensionAction || !command.isActive } triggerScopeChange_(scope) { return scope } onCloseButtonClick_() { this.dispatchEvent(new CustomEvent("close", { bubbles: true, composed: true })) } onScopeChanged_(event) { this.delegate.updateExtensionCommandScope(event.model.get("item.id"), event.model.get("command.name"), event.target.value) } } customElements.define(ExtensionsKeyboardShortcutsElement.is, ExtensionsKeyboardShortcutsElement); function getTemplate$c() {
...
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
class ExtensionsLoadErrorElement extends PolymerElement { static get is() { return "extensions-load-error" } static get template() { return getTemplate$c() } static get properties() { return { delegate: Object, loadError: Object, retrying_: Boolean } } static get observers() { return ["observeLoadErrorChanges_(loadError)"] } show() { this.$.dialog.showModal() } close() { this.$.dialog.close() } onRetryTap_() { this.retrying_ = true; this.delegate.retryLoadUnpacked(this.loadError.retryGuid).then((() => { this.close() }), (loadError => { this.loadError = loadError; this.retrying_ = false })) } observeLoadErrorChanges_() { assert$1(this.loadError); const source = this.loadError.source; const codeSectionProperties = { beforeHighlight: source ? source.beforeHighlight : "", highlight: source ? source.highlight : "", afterHighlight: source ? source.afterHighlight : "", title: "", message: this.loadError.error }; this.$.code.code = codeSectionProperties } } customElements.define(ExtensionsLoadErrorElement.is, ExtensionsLoadErrorElement); function getTemplate$b() {
      ...
      <div class$="[[getDialogBodyContainerClass_(site)]]">
        <template is="dom-repeat" items="[[extensionSiteAccessData_]]">
          <div class="extension-row">
            ...
            <select class="extension-host-access md-select" value="[[item.siteAccess]]" disabled="disabled">
...
<!--_html_template_end_-->`}
// Copyright 2022 The Chromium Authors
const EXTENSION_SPECIFIED = chrome.developerPrivate.SiteSet.EXTENSION_SPECIFIED; const VALID_SCHEMES = ["*", "http", "https", "file", "ftp", "chrome", "chrome-extension", "filesystem", "ftp", "ws", "wss", "data", "uuid-in-package"]; const SitePermissionsEditPermissionsDialogElementBase = I18nMixin(PolymerElement); class SitePermissionsEditPermissionsDialogElement extends SitePermissionsEditPermissionsDialogElementBase { static get is() { return "site-permissions-edit-permissions-dialog" } static get template() { return getTemplate$9() } static get properties() { return { delegate: Object, extensions: { type: Array, value: () => [], observer: "onExtensionsUpdated_" }, originalSiteSet: String, site: String, siteSet_: { type: String, observer: "onSiteSetUpdated_" }, siteSetEnum_: { type: Object, value: chrome.developerPrivate.SiteSet }, extensionSiteAccessData_: { type: Array, value: () => [] }, hostAccessEnum_: { type: Object, value: chrome.developerPrivate.HostAccess } } } ready() { super.ready(); this.siteSet_ = this.originalSiteSet; assert$1(!this.matchesSubdomains_() || this.originalSiteSet === EXTENSION_SPECIFIED); this.updateExtensionSiteAccessData_(this.siteSet_) } onExtensionsUpdated_(extensions) { this.extensionsIdToInfo_ = new Map; for (const extension of extensions) { this.extensionsIdToInfo_.set(extension.id, extension) } this.updateExtensionSiteAccessData_(this.siteSet_) } onSiteSetUpdated_(siteSet) { this.updateExtensionSiteAccessData_(siteSet) } isSiteHostOnly_() { return !VALID_SCHEMES.some((scheme => this.site.startsWith(`${scheme}://`))) } async updateExtensionSiteAccessData_(siteSet) { if (siteSet !== EXTENSION_SPECIFIED) { return } const siteToCheck = this.isSiteHostOnly_() ? `*://${this.site}/` : this.site; const matchingExtensionsInfo = await this.delegate.getMatchingExtensionsForSite(siteToCheck); const extensionSiteAccessData = []; matchingExtensionsInfo.forEach((({ id: id, siteAccess: siteAccess }) => { assert$1(this.extensionsIdToInfo_.has(id)); const { name: name, iconUrl: iconUrl } = this.extensionsIdToInfo_.get(id); extensionSiteAccessData.push({ id: id, name: name, iconUrl: iconUrl, siteAccess: siteAccess }) })); this.extensionSiteAccessData_ = extensionSiteAccessData } onCancelClick_() { this.$.dialog.cancel() } onSubmitClick_() { if (this.siteSet_ === this.originalSiteSet) { this.$.dialog.close(); return } const sitesToChange = this.isSiteHostOnly_() ? [`http://${this.site}`, `https://${this.site}`] : [this.site]; if (this.siteSet_ === EXTENSION_SPECIFIED) { this.delegate.removeUserSpecifiedSites(this.originalSiteSet, sitesToChange).then((() => { this.$.dialog.close() })) } else { this.delegate.addUserSpecifiedSites(this.siteSet_, sitesToChange).then((() => { this.$.dialog.close() })) } } computeDialogTitle_() { return this.i18n("sitePermissionsEditPermissionsDialogTitle", this.site) } getPermittedSiteLabel_() { return this.i18n("editSitePermissionsAllowAllExtensions", this.site) } getRestrictedSiteLabel_() { return this.i18n("editSitePermissionsRestrictExtensions", this.site) } matchesSubdomains_() { return this.site.includes("*.") } showExtensionSiteAccessData_() { return this.siteSet_ === EXTENSION_SPECIFIED } getDialogBodyContainerClass_() { return this.matchesSubdomains_() ? "site-access-list" : "indented-site-access-list" } } customElements.define(SitePermissionsEditPermissionsDialogElement.is, SitePermissionsEditPermissionsDialogElement); function getTemplate$8() {
...
<!--_html_template_end_-->`}
// Copyright 2022 The Chromium Authors
class ExtensionsSitePermissionsListElement extends PolymerElement { constructor() { super(...arguments); this.siteToEditAnchorElement_ = null } static get is() { return "site-permissions-list" } static get template() { return getTemplate$8() } static get properties() { return { delegate: Object, extensions: Array, header: String, siteSet: String, sites: Array, showEditSiteUrlDialog_: { type: Boolean, value: false }, showEditSitePermissionsDialog_: { type: Boolean, value: false }, siteToEdit_: { type: String, value: null } } } hasSites_() { return !!this.sites.length } getFaviconUrl_(url) { return getFaviconUrl(url) } focusOnAnchor_() { assert$1(this.siteToEditAnchorElement_, "Site Anchor"); focusWithoutInk(this.siteToEditAnchorElement_); this.siteToEditAnchorElement_ = null } onAddSiteClick_() { assert$1(!this.showEditSitePermissionsDialog_); this.siteToEdit_ = null; this.showEditSiteUrlDialog_ = true } onEditSiteUrlDialogClose_() { this.showEditSiteUrlDialog_ = false; if (this.siteToEdit_ !== null) { this.focusOnAnchor_() } this.siteToEdit_ = null } onEditSitePermissionsDialogClose_() { this.showEditSitePermissionsDialog_ = false; assert$1(this.siteToEdit_, "Site To Edit"); this.focusOnAnchor_(); this.siteToEdit_ = null } onDotsClick_(e) { this.siteToEdit_ = e.model.item; assert$1(!this.showEditSitePermissionsDialog_); this.$.siteActionMenu.showAt(e.target); this.siteToEditAnchorElement_ = e.target } onEditSitePermissionsClick_() { this.closeActionMenu_(); assert$1(this.siteToEdit_ !== null); this.showEditSitePermissionsDialog_ = true } onEditSiteUrlClick_() { this.closeActionMenu_(); assert$1(this.siteToEdit_ !== null); this.showEditSiteUrlDialog_ = true } onRemoveSiteClick_() { assert$1(this.siteToEdit_, "Site To Edit"); this.delegate.removeUserSpecifiedSites(this.siteSet, [this.siteToEdit_]).then((() => { this.closeActionMenu_(); this.siteToEdit_ = null })) } closeActionMenu_() { const menu = this.$.siteActionMenu; assert$1(menu.open); menu.close() } } customElements.define(ExtensionsSitePermissionsListElement.is, ExtensionsSitePermissionsListElement); function getTemplate$7() {
...
<!--_html_template_end_-->`}
// Copyright 2021 The Chromium Authors
const ExtensionsSitePermissionsElementBase = SiteSettingsMixin(PolymerElement); class ExtensionsSitePermissionsElement extends ExtensionsSitePermissionsElementBase { static get is() { return "extensions-site-permissions" } static get template() { return getTemplate$7() } static get properties() { return { extensions: Array, siteSetEnum_: { type: Object, value: chrome.developerPrivate.SiteSet } } } onAllSitesLinkClick_() { navigation.navigateTo({ page: Page.SITE_PERMISSIONS_ALL_SITES }) } } customElements.define(ExtensionsSitePermissionsElement.is, ExtensionsSitePermissionsElement); function getTemplate$6() {
    return html`<!--_html_template_start_--><style include="cr-shared-style cr-icons shared-style">#etld-row{align-items:center;display:flex;height:var(--cr-section-two-line-min-height)}#etld-and-subtext{display:flex;flex-direction:column;flex-grow:1;margin:0 calc(var(--cr-section-padding) + var(--cr-icon-ripple-margin));overflow:hidden;text-overflow:ellipsis}#etldOrSiteSubtext{color:var(--cr-secondary-text-color)}#sites-list{margin-inline-start:var(--cr-section-indent-padding)}.site-row{align-items:center;display:flex;height:var(--cr-section-min-height)}.site-and-subtext{display:flex;flex-grow:1;margin:0 calc(var(--cr-section-padding) + var(--cr-icon-ripple-margin));overflow:hidden}.site{margin-inline-end:12px;overflow:hidden;text-overflow:ellipsis}.site-subtext{color:var(--cr-secondary-text-color);flex-shrink:0}</style>
...
<!--_html_template_end_-->`}
// Copyright 2022 The Chromium Authors
class ExtensionsSitePermissionsBySiteElement extends PolymerElement { static get is() { return "extensions-site-permissions-by-site" } static get template() { return getTemplate$5() } static get properties() { return { delegate: Object, extensions: Array, siteGroups_: { type: Array, value: () => [] } } } ready() { super.ready(); this.refreshUserAndExtensionSites_(); this.delegate.getUserSiteSettingsChangedTarget().addListener(this.refreshUserAndExtensionSites_.bind(this)); this.delegate.getItemStateChangedTarget().addListener(this.refreshUserAndExtensionSites_.bind(this)) } refreshUserAndExtensionSites_() { this.delegate.getUserAndExtensionSitesByEtld().then((sites => { this.siteGroups_ = sites })) } onCloseButtonClick_() { navigation.navigateTo({ page: Page.SITE_PERMISSIONS }) } } customElements.define(ExtensionsSitePermissionsBySiteElement.is, ExtensionsSitePermissionsBySiteElement); function getTemplate$4() {
...
<!--_html_template_end_-->`}
// Copyright 2017 The Chromium Authors
class ExtensionsPackDialogAlertElement extends PolymerElement { constructor() { super(...arguments); this.cancelLabel_ = null; this.confirmLabel_ = "" } static get is() { return "extensions-pack-dialog-alert" } static get template() { return getTemplate$4() } static get properties() { return { model: Object, title_: String, message_: String, cancelLabel_: String, confirmLabel_: String } } get returnValue() { return this.$.dialog.getNative().returnValue } ready() { super.ready(); this.cancelLabel_ = null; this.confirmLabel_ = null; switch (this.model.status) { case chrome.developerPrivate.PackStatus.WARNING: this.title_ = loadTimeData.getString("packDialogWarningTitle"); this.cancelLabel_ = loadTimeData.getString("cancel"); this.confirmLabel_ = loadTimeData.getString("packDialogProceedAnyway"); break; case chrome.developerPrivate.PackStatus.ERROR: this.title_ = loadTimeData.getString("packDialogErrorTitle"); this.cancelLabel_ = loadTimeData.getString("ok"); break; case chrome.developerPrivate.PackStatus.SUCCESS: this.title_ = loadTimeData.getString("packDialogTitle"); this.cancelLabel_ = loadTimeData.getString("ok"); break; default: assertNotReached$1() } } connectedCallback() { super.connectedCallback(); this.$.dialog.showModal() } getCancelButtonClass_() { return this.confirmLabel_ ? "cancel-button" : "action-button" } onCancelTap_() { this.$.dialog.cancel() } onConfirmTap_() { assert$1(this.model.status === chrome.developerPrivate.PackStatus.WARNING); this.$.dialog.close() } } customElements.define(ExtensionsPackDialogAlertElement.is, ExtensionsPackDialogAlertElement); function getTemplate$3() {
...
function compareExtensions(a, b) { function compare(x, y) { return x < y ? -1 : x > y ? 1 : 0 } function compareLocation(x, y) { if (x.location === y.location) { return 0 } if (x.location === chrome.developerPrivate.Location.UNPACKED) { return -1 } if (y.location === chrome.developerPrivate.Location.UNPACKED) { return 1 } return 0 } return compareLocation(a, b) || compare(a.name.toLowerCase(), b.name.toLowerCase()) || compare(a.id, b.id) } class ExtensionsManagerElement extends PolymerElement { constructor() { super(); this.navigationListener_ = null; this.currentPage_ = null; this.navigationListener_ = null } static get is() { return "extensions-manager" } static get template() { return getTemplate$1() } static get properties() { return { canLoadUnpacked: { type: Boolean, value: false }, delegate: { type: Object, value() { return Service.getInstance() } }, inDevMode: { type: Boolean, value: () => loadTimeData.getBoolean("inDevMode") }, showActivityLog: { type: Boolean, value: () => loadTimeData.getBoolean("showActivityLog") }, enableEnhancedSiteControls: { type: Boolean, value: () => loadTimeData.getBoolean("enableEnhancedSiteControls") }, devModeControlledByPolicy: { type: Boolean, value: false }, isChildAccount_: { type: Boolean, value: false }, incognitoAvailable_: { type: Boolean, value: false }, filter: { type: String, value: "" }, errorPageItem_: Object, detailViewItem_: Object, activityLogItem_: Object, extensions_: Array, apps_: Array, didInitPage_: { type: Boolean, value: false }, showDrawer_: Boolean, showLoadErrorDialog_: Boolean, showInstallWarningsDialog_: Boolean, installWarnings_: Array, showOptionsDialog_: Boolean, fromActivityLog_: Boolean } } ready() { super.ready(); this.addEventListener("load-error", this.onLoadError_); this.addEventListener("view-enter-start", this.onViewEnterStart_); this.addEventListener("view-exit-start", this.onViewExitStart_); this.addEventListener("view-exit-finish", this.onViewExitFinish_); const service = Service.getInstance(); const onProfileStateChanged = profileInfo => { this.isChildAccount_ = profileInfo.isChildAccount; this.incognitoAvailable_ = profileInfo.isIncognitoAvailable; this.devModeControlledByPolicy = profileInfo.isDeveloperModeControlledByPolicy; this.inDevMode = profileInfo.inDeveloperMode; this.canLoadUnpacked = profileInfo.canLoadUnpacked }; service.getProfileStateChangedTarget().addListener(onProfileStateChanged); service.getProfileConfiguration().then(onProfileStateChanged); service.getExtensionsInfo().then((extensionsAndApps => { this.initExtensionsAndApps_(extensionsAndApps); this.initPage_(); service.getItemStateChangedTarget().addListener(this.onItemStateChanged_.bind(this)) })) } connectedCallback() { super.connectedCallback(); document.documentElement.classList.remove("loading"); document.fonts.load("bold 12px Roboto"); this.navigationListener_ = navigation.addListener((newPage => { this.changePage_(newPage) })) } disconnectedCallback() { super.disconnectedCallback(); assert$1(this.navigationListener_); assert$1(navigation.removeListener(this.navigationListener_)); this.navigationListener_ = null } initPage_() { this.didInitPage_ = true; this.changePage_(navigation.getCurrentPage()) } onItemStateChanged_(eventData) { const EventType = chrome.developerPrivate.EventType; switch (eventData.event_type) { case EventType.VIEW_REGISTERED: case EventType.VIEW_UNREGISTERED: case EventType.INSTALLED: case EventType.LOADED: case EventType.UNLOADED: case EventType.ERROR_ADDED: case EventType.ERRORS_REMOVED: case EventType.PREFS_CHANGED: case EventType.WARNINGS_CHANGED: case EventType.COMMAND_ADDED: case EventType.COMMAND_REMOVED: case EventType.PERMISSIONS_CHANGED: case EventType.SERVICE_WORKER_STARTED: case EventType.SERVICE_WORKER_STOPPED: if (!eventData.extensionInfo) { break } if (this.delegate.shouldIgnoreUpdate(eventData.extensionInfo.id, eventData.event_type)) { break } const listId = this.getListId_(eventData.extensionInfo); const currentIndex = this.get(listId).findIndex((item => item.id === eventData.extensionInfo.id)); if (currentIndex >= 0) { this.updateItem_(listId, currentIndex, eventData.extensionInfo) } else { this.addItem_(listId, eventData.extensionInfo) } break; case EventType.UNINSTALLED: this.removeItem_(eventData.item_id); break; default: assertNotReached$1() } } onFilterChanged_(event) { if (this.currentPage_.page !== Page.LIST) { navigation.navigateTo({ page: Page.LIST }) } this.filter = event.detail } onMenuButtonTap_() { this.showDrawer_ = true; setTimeout((() => { this.shadowRoot.querySelector("cr-drawer").openDrawer() }), 0) } getListId_(item) { const ExtensionType = chrome.developerPrivate.ExtensionType; switch (item.type) { case ExtensionType.HOSTED_APP: case ExtensionType.LEGACY_PACKAGED_APP: case ExtensionType.PLATFORM_APP: return "apps_"; case ExtensionType.EXTENSION: case ExtensionType.SHARED_MODULE: return "extensions_"; case ExtensionType.THEME: assertNotReached$1("Don't send themes to the chrome://extensions page"); default: assertNotReached$1() } } getIndexInList_(listId, itemId) { return this.get(listId).findIndex((function (item) { return item.id === itemId })) } getData_(id) { return this.extensions_[this.getIndexInList_("extensions_", id)] || this.apps_[this.getIndexInList_("apps_", id)] } initExtensionsAndApps_(extensionsAndApps) { extensionsAndApps.sort(compareExtensions); const apps = []; const extensions = []; for (const i of extensionsAndApps) { const list = this.getListId_(i) === "apps_" ? apps : extensions; list.push(i) } this.apps_ = apps; this.extensions_ = extensions } addItem_(listId, item) { assert$1(this.getIndexInList_(listId, item.id) === -1); let insertBeforeChild = this.get(listId).findIndex((function (listEl) { return compareExtensions(listEl, item) > 0 })); if (insertBeforeChild === -1) { insertBeforeChild = this.get(listId).length } this.splice(listId, insertBeforeChild, 0, item) } updateItem_(listId, index, item) { assert$1(index >= 0); this.set([listId, index], item); if (this.detailViewItem_ && this.detailViewItem_.id === item.id && this.currentPage_.page === Page.DETAILS) { this.detailViewItem_ = item } else if (this.errorPageItem_ && this.errorPageItem_.id === item.id && this.currentPage_.page === Page.ERRORS) { this.errorPageItem_ = item } else if (this.activityLogItem_ && this.activityLogItem_.id === item.id && this.currentPage_.page === Page.ACTIVITY_LOG) { this.activityLogItem_ = item } } removeItem_(itemId) { let listId = "extensions_"; let index = this.getIndexInList_(listId, itemId); if (index === -1) { listId = "apps_"; index = this.getIndexInList_(listId, itemId) } assert$1(index >= 0); this.splice(listId, index, 1); if ((this.currentPage_.page === Page.ACTIVITY_LOG || this.currentPage_.page === Page.DETAILS || this.currentPage_.page === Page.ERRORS) && this.currentPage_.extensionId === itemId) { navigation.replaceWith({ page: Page.LIST }) } } onLoadError_(e) { this.showLoadErrorDialog_ = true; setTimeout((() => { const dialog = this.shadowRoot.querySelector("extensions-load-error"); dialog.loadError = e.detail; dialog.show() }), 0) } changePage_(newPage) { this.onCloseDrawer_(); const optionsDialog = this.shadowRoot.querySelector("extensions-options-dialog"); if (optionsDialog && optionsDialog.open) { this.showOptionsDialog_ = false } const fromPage = this.currentPage_ ? this.currentPage_.page : null; const toPage = newPage.page; let data; let activityLogPlaceholder; if (newPage.extensionId) { data = this.getData_(newPage.extensionId); if (!data) { if (this.showActivityLog && toPage === Page.ACTIVITY_LOG) { activityLogPlaceholder = { id: newPage.extensionId, isPlaceholder: true } } else { navigation.replaceWith({ page: Page.LIST }); return } } } if (toPage === Page.DETAILS) { this.detailViewItem_ = data } else if (toPage === Page.ERRORS) { this.errorPageItem_ = data } else if (toPage === Page.ACTIVITY_LOG) { if (!this.showActivityLog) { navigation.replaceWith({ page: Page.DETAILS, extensionId: newPage.extensionId }); return } this.activityLogItem_ = data || activityLogPlaceholder } else if ((toPage === Page.SITE_PERMISSIONS || toPage === Page.SITE_PERMISSIONS_ALL_SITES) && !this.enableEnhancedSiteControls) { navigation.replaceWith({ page: Page.LIST }); return } if (fromPage !== toPage) { this.$.viewManager.switchView(toPage) } if (newPage.subpage) { assert$1(newPage.subpage === Dialog.OPTIONS); assert$1(newPage.extensionId); this.showOptionsDialog_ = true; setTimeout((() => { this.shadowRoot.querySelector("extensions-options-dialog").show(data) }), 0) } document.title = toPage === Page.DETAILS ? `${loadTimeData.getString("title")} - ${this.detailViewItem_.name}` : loadTimeData.getString("title"); this.currentPage_ = newPage } onDrawerClose_() { this.showDrawer_ = false } onCloseDrawer_() { const drawer = this.shadowRoot.querySelector("cr-drawer"); if (drawer && drawer.open) { drawer.close() } } onLoadErrorDialogClose_() { this.showLoadErrorDialog_ = false } onOptionsDialogClose_() { this.showOptionsDialog_ = false; this.shadowRoot.querySelector("extensions-detail-view").focusOptionsButton() } onViewEnterStart_() { this.fromActivityLog_ = false } onViewExitStart_(e) { const viewType = e.composedPath()[0].tagName; this.fromActivityLog_ = viewType === "EXTENSIONS-ACTIVITY-LOG" } onViewExitFinish_(e) { const viewType = e.composedPath()[0].tagName; if (viewType === "EXTENSIONS-ITEM-LIST" || viewType === "EXTENSIONS-KEYBOARD-SHORTCUTS" || viewType === "EXTENSIONS-ACTIVITY-LOG" || viewType === "EXTENSIONS-SITE-PERMISSIONS" || viewType === "EXTENSIONS-SITE-PERMISSIONS-BY-SITE") { return } const extensionId = e.composedPath()[0].data.id; const list = this.shadowRoot.querySelector("extensions-item-list"); const button = viewType === "EXTENSIONS-DETAIL-VIEW" ? list.getDetailsButton(extensionId) : list.getErrorsButton(extensionId); if (button) { button.focus() } } onShowInstallWarnings_(e) { this.installWarnings_ = e.detail; this.showInstallWarningsDialog_ = true } onInstallWarningsDialogClose_() { this.installWarnings_ = null; this.showInstallWarningsDialog_ = false } } customElements.define(ExtensionsManagerElement.is, ExtensionsManagerElement); function getTemplate() {
    return html`<!--_html_template_start_-->    <style>:host{-webkit-tap-highlight-color:transparent;align-items:center;cursor:pointer;display:flex;outline:0;user-select:none;--cr-checkbox-border-size:2px;--cr-checkbox-size:16px;--cr-checkbox-ripple-size:40px;--cr-checkbox-ripple-offset:calc(var(--cr-checkbox-size)/2 -
            var(--cr-checkbox-ripple-size)/2 - var(--cr-checkbox-border-size));--cr-checkbox-checked-box-color:var(--cr-checked-color);--cr-checkbox-ripple-checked-color:var(--cr-checked-color);--cr-checkbox-checked-ripple-opacity:.2;--cr-checkbox-mark-color:white;--cr-checkbox-ripple-unchecked-color:var(--google-grey-900);--cr-checkbox-unchecked-box-color:var(--google-grey-700);--cr-checkbox-unchecked-ripple-opacity:.15}@media (prefers-color-scheme:dark){:host{--cr-checkbox-checked-ripple-opacity:.4;--cr-checkbox-mark-color:var(--google-grey-900);--cr-checkbox-ripple-unchecked-color:var(--google-grey-500);--cr-checkbox-unchecked-box-color:var(--google-grey-500);--cr-checkbox-unchecked-ripple-opacity:.4}}:host([disabled]){cursor:initial;opacity:var(--cr-disabled-opacity);pointer-events:none}#checkbox{background:0 0;border:var(--cr-checkbox-border-size) solid var(--cr-checkbox-unchecked-box-color);border-radius:2px;box-sizing:border-box;cursor:pointer;display:block;flex-shrink:0;height:var(--cr-checkbox-size);margin:0;outline:0;padding:0;position:relative;transform:none;width:var(--cr-checkbox-size)}@media (forced-colors:active){:host(:focus) #checkbox{outline:var(--cr-focus-outline-hcm)}}#checkmark{border-color:var(--cr-checkbox-mark-color);border-style:solid;border-width:0 2px 2px 0;content:'';display:block;height:73%;transform:scale(0) rotate(45deg);transform-origin:100% 80%;width:36%}:host-context([dir=rtl]) #checkmark{transform-origin:50% 14%}:host([checked]) #checkbox{background:var(--cr-checkbox-checked-box-background-color,var(--cr-checkbox-checked-box-color));border-color:var(--cr-checkbox-checked-box-color)}:host([checked]) #checkmark{transform:scale(1) rotate(45deg);transition:transform 140ms ease-out}paper-ripple{--paper-ripple-opacity:var(--cr-checkbox-ripple-opacity,
...
<!--_html_template_end_-->`}
// Copyright 2018 The Chromium Authors
const CrCheckboxElementBase = mixinBehaviors([PaperRippleBehavior], PolymerElement); class CrCheckboxElement extends CrCheckboxElementBase { static get is() { return "cr-checkbox" } static get template() { return getTemplate() } static get properties() { return { checked: { type: Boolean, value: false, reflectToAttribute: true, observer: "checkedChanged_", notify: true }, disabled: { type: Boolean, value: false, reflectToAttribute: true, observer: "disabledChanged_" }, ariaDescription: String, tabIndex: { type: Number, value: 0, observer: "onTabIndexChanged_" } } } ready() { super.ready(); this.removeAttribute("unresolved"); this.addEventListener("blur", this.hideRipple_.bind(this)); this.addEventListener("click", this.onClick_.bind(this)); this.addEventListener("focus", this.showRipple_.bind(this)); this.addEventListener("up", this.hideRipple_.bind(this)) } focus() { this.$.checkbox.focus() } getFocusableElement() { return this.$.checkbox } checkedChanged_() { this.$.checkbox.setAttribute("aria-checked", this.checked ? "true" : "false") } disabledChanged_(_current, previous) { if (previous === undefined && !this.disabled) { return } this.tabIndex = this.disabled ? -1 : 0; this.$.checkbox.setAttribute("aria-disabled", this.disabled ? "true" : "false") } showRipple_() { this.getRipple().showAndHoldDown() } hideRipple_() { this.getRipple().clear() } onClick_(e) { if (this.disabled || e.target.tagName === "A") { return } e.stopPropagation(); e.preventDefault(); this.checked = !this.checked; this.dispatchEvent(new CustomEvent("change", { bubbles: true, composed: true, detail: this.checked })) } onKeyDown_(e) { if (e.key !== " " && e.key !== "Enter") { return } e.preventDefault(); e.stopPropagation(); if (e.repeat) { return } if (e.key === "Enter") { this.click() } } onKeyUp_(e) { if (e.key === " " || e.key === "Enter") { e.preventDefault(); e.stopPropagation() } if (e.key === " ") { this.click() } } onTabIndexChanged_() { this.removeAttribute("tabindex") } _createRipple() { this._rippleContainer = this.$.checkbox; const ripple = super._createRipple(); ripple.id = "ink"; ripple.setAttribute("recenters", ""); ripple.classList.add("circle", "toggle-ink"); return ripple } } customElements.define(CrCheckboxElement.is, CrCheckboxElement); export { ARG_URL_PLACEHOLDER, ActivityLogHistoryElement, ActivityLogHistoryItemElement, ActivityLogPageState, ActivityLogStreamElement, ActivityLogStreamItemElement, CrCheckboxElement, Dialog, ExtensionsActivityLogElement, ExtensionsCodeSectionElement, ExtensionsDetailViewElement, ExtensionsErrorPageElement, ExtensionsHostPermissionsToggleListElement, ExtensionsItemElement, ExtensionsItemListElement, ExtensionsKeyboardShortcutsElement, ExtensionsLoadErrorElement, ExtensionsManagerElement, ExtensionsOptionsDialogElement, ExtensionsPackDialogAlertElement, ExtensionsPackDialogElement, ExtensionsRestrictedSitesDialogElement, ExtensionsRuntimeHostPermissionsElement, ExtensionsRuntimeHostsDialogElement, ExtensionsShortcutInputElement, ExtensionsSidebarElement, ExtensionsSitePermissionsBySiteElement, ExtensionsSitePermissionsElement, ExtensionsSitePermissionsListElement, ExtensionsToggleRowElement, ExtensionsToolbarElement, Key, NavigationHelper, OptionsDialogMaxHeight, OptionsDialogMinWidth, Page, Service, SitePermissionsEditPermissionsDialogElement, SitePermissionsEditUrlDialogElement, SitePermissionsSiteGroupElement, SiteSettingsMixin, UserAction, getFaviconUrl, getMatchingUserSpecifiedSites, getPatternFromSite, getSitePermissionsPatternFromSite, getToastManager, isValidKeyCode, keystrokeToString, navigation };